...
API_TOKEN = '7395613360:AAEhdVxsd1Vo6Ip920-o8koH1sZKB2Bnud0'
...
async def remove_user(message: Message):
    if message.from_user.id!= ADMIN_ID:
        await message.answer("😂 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 😒🤣")
        return
    args = message.text.split()[1:]
    if len(args)!= 1:
...
async def udpbgmi_attack(message: Message):
    if not await check_authorization(message.from_user.id):
        await message.answer("𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n 𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @SH4D0WO 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨")
        return
...
async def udpbgmi_stop(message: Message):
    if not await check_authorization(message.from_user.id):
        await message.answer("𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n 𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @SH4D0WO 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨")
        return
    if message.from_user.id not in AUTHORIZED_USERS:
...
async def main():
    ...
    dp.message.register(udpbgmi_attack, filters.Command(commands=['udpbgmi']))
    dp.message.register(broadcast, filters.Command("broadcast"))
    dp.message.register(udpbgmi_stop, filters.Command("stop"))
